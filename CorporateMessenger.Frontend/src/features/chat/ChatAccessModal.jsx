...
import { getRoles, getChatAccessRules, grantChatAccess, revokeChatAccess } from '../../services/api';

const CHAT_ACCESS_PERMISSIONS = [
  { value: 1, label: 'Чтение сообщений' },     // ReadMessages
  { value: 2, label: 'Реакции на сообщения' }, // ReactMessages
];

function ChatAccessModal({ isOpen, onClose, chatId }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const currentMask = accessRules[selectedRoleId] || 0;
      const previousMask = (await getChatAccessRules(chatId)).find(
        (rule) => rule.roleId === selectedRoleId
      )?.accessMask || 0;

      // Determine permissions to grant or revoke
      for (const perm of CHAT_ACCESS_PERMISSIONS) {
        ...
        if (isSetNow && !wasSetBefore) {
          await grantChatAccess(chatId, { roleId: selectedRoleId, access: perm.value });
        } else if (!isSetNow && wasSetBefore) {
          await revokeChatAccess(chatId, { roleId: selectedRoleId, access: perm.value });
        }
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Настройка доступа к чату</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <VStack spacing={4} align="start">
            ...
            {selectedRole && (
              <Box>
                ...
                {CHAT_ACCESS_PERMISSIONS.map((perm) => (
                  <Checkbox
                    key={perm.value}
